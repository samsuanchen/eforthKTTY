...
;  建 INTERUPT VECTOR CALL表
							;向量號碼，向量名稱
	JMP			ORIG		;1, RESET ( 4 byte JUMP )
	CALL		ISR_HEAD	;2, INT0
	CALL		ISR_HEAD	;3, INT1
	CALL		ISR_HEAD	;4, PC0
	CALL		ISR_HEAD	;5, PC1
	CALL		ISR_HEAD	;6, PC2
	CALL		ISR_HEAD	;7, WDT
	CALL		ISR_HEAD	;8, T2A
	CALL		ISR_HEAD	;9, T2B
	CALL		ISR_HEAD	;10,T2OV
	CALL		ISR_HEAD	;11,T1CAP
	CALL		ISR_HEAD	;12,T1A
	CALL		ISR_HEAD	;13,T1B
	CALL		ISR_HEAD	;14,T1OV
	CALL		ISR_HEAD	;15,T0A
	CALL		ISR_HEAD	;16,T0B
	CALL		ISR_HEAD	;17,T0OV
	CALL		ISR_HEAD	;18,SPI
	CALL		ISR_HEAD	;19,URX
	CALL		ISR_HEAD	;20,UDE
	CALL		ISR_HEAD	;21,UTX
	CALL		ISR_HEAD	;22,ADC
	CALL		ISR_HEAD	;23,EERDY
	CALL		ISR_HEAD	;24,CMP
	CALL		ISR_HEAD	;25,TWI
	CALL		ISR_HEAD	;26,SPMRDY
; = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = 
; $34 ~ $64  = #2 ~ #26 
...
;      
ISR_HEAD:	; 會改變 R4, R5 
	IN  IVtL, SREG ; 將 SREG 備存
	POP IVaH ; 忽略 return address high byte
	POP IVaL ; 取出 return address low  byte
	LSR IVaL ; 除 2 (每個 interrupt 佔 1 word)
	CLC
	ADD IVaL,IVa0 ; 算出 高階 interrupt vector 表位
	OUT SREG, IVtL ; 將 SREG 還原
	RET ; 
...
INT_handler:
	...
	MOV xl, IVaL
	CLR xh
	LD IVaL, X+ ; 高階 interrupt service routine 位址
	LD IVaH, X
	MOV xh, IVtH ; 將 x 還原
	MOV xl, IVtL
	PUSH IVaL
...
QKEY:
	savetos
	clr 	tosl		; 20131121 sam 應可刪除
	clr 	tosh		; 20131121 sam 應可刪除
	movw	tosl,zerol
	in_		xl,UCSR0A
	sbrs	xl,7
	jmp INT_handler
...
UPLUS:
	...
	adc 	tosh, xh
	savetos
	clr		tosh		; 20131121 sam 應可更改
	clr		tosl		; 20131121 sam 應可更改
...
;	Filter non-printing characters.
	...
	CODE	5,">CHAR"
TCHAR:
    jmp INT_handler           ;  No filter ;albert  2012/5/2
	RCALL	DUPP
	RCALL	BLANK
	RCALL	DOLIT
	.DW		$7F
...
EXE1:
	jmp INT_handler				;do nothing if zero
...
;	Change character to upper case
	...
	CODE	5,"UPPER"
UPPER:
   jmp INT_handler      ; NO capitalize ; albert  2012/5/2 
	RCALL	DUPP
	RCALL	DOLIT
	.DW		$61
	RCALL	DOLIT
...
STRQP:
	RCALL	DOSTR
	jmp INT_handler				;force a call to do$
...
ABORT:
	...
	CODE	12,"ERR_HANDLING" ; ADD FOR CHANGE 8 / 23  
ERR_HANDLING:
    JMP  RAT	; to be replaced
    
	CODE	10,"ALLOW_BOOT" ;  ADD FOR CHANGE 8 / 30 
ALLOW_BOOT:
    JMP  RAT	; to be replaced
	jmp INT_handler	; 20131127 應刪除
...
INTER:
	...
	.DB		13," compile only"
	RCALL	EXECU
	jmp INT_handler	;execute defined word
INTE1:
	RCALL	NUMBQ
	RCALL	QBRAN
	.DW		INTE2
...
TICK:
	...
	RCALL	QBRAN
	.DW		ERROR
	jmp INT_handler				;yes, push code address
TICK1:
	RJMP	ERROR	;no, error
...
; 	transfer data from RAM to Flash page buffer
	CODE	5,"WRITE";; Device dependent I/O
	JMP		WRITE
...
;	Compile the next address in colon list to code dictionary.
	...
	CODE	COMPO+7,"COMPILE"
COMPI:
	CALL	RFROM
	CALL	CELLS
	CALL	DUPP
	CALL	IAT		;fixed 20131107 sam
;	CALL	AT
...
;	CALL	AT
	...
	CALL	TWOSL
	CALL	TOR
	jmp INT_handler				;adjust return address

;   LITERAL	( w -- )
;	Compile tos to code dictionary as an integer literal.

	CODE  IMEDD+7,"LITERAL" ; FIXED 2012/9/25
LITER: 
	CALL	DOLIT
	.DW		DOLIT
	CALL	CALLC
